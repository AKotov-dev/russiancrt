...
**Примечание:** В Mageia/Fedora - Firefox берёт настройки сам из системного хранилища; для Chromium-браузеров нужно зайти в менеджер сертификатов например `brave://certificate-manager/`, выключить переключатель `Использовать локальные сертификаты, импортированные из операционной системы` и добавить сертификаты в `Установленные вами`. В Ubuntu/Debian - на уровне системы; в браузерах сертификаты указываются вручную.
