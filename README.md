...
* После перехода на системное хранилище **необходимо удалить импортированные ранее сертификаты** и перезапустить Firefox. Теперь он будет читать их автоматически.
...
Если ранее XCA не использовалась, создайте новую базу данных: `Меню - Файл - Новая база данных`.

### Шаг 1. Создаём `Secured-Private-Root`

На вкладке **Закрытые ключи** жмём **ПКМ** - **Новый** (ключ) с именем `Secured-Private-Root`.
