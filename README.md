## russiancrt
Пакет RPM для установки безопасных сертификатов Минцифры.  
...
Все последующие обновления сертификатов в рамках данного пакета также будут обрабатываться этим механизмом.  
  
Генерация модифицированных сертификатов выполняется скриптом, входящим в состав репозитория. Исходный закрытый ключ УЦ не используется и не требуется.  
