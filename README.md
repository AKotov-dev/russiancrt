...
* **Debian/Ubuntu — системный уровень** — сертификаты устанавливаются в `/usr/local/share/ca-certificates/`, после чего выполняется:

  ```bash
  sudo update-ca-certificates
  ```

  После этого они доступны системному trust store. Проверить работу ограничений можно, например, через `openssl s_client`: для разрешённой зоны (`.ru`, `.su`, `.рф`) проверка проходит, а для запрещённой (`.com`) возникает `permitted subtree violation`.
...
* После перехода с ручной установки сертификатов на системное хранилище **необходимо удалить ранее вручную импортированные копии сертификатов из Firefox (`cert9.db`)**. Иначе Firefox может построить альтернативный доверенный путь и обойти ограничения `Name Constraints`.
